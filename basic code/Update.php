...
if (isset($_POST['Submit'])) {
		...
		$ConnectingDB;
		$sql = "UPDATE emp_record SET ename='$EName',ssn= '$SSN',dept='$Depart',salary='$Salary',
		homeaddress='$HomeAddress' WHERE id='$SearchQueryParameter'";

		$Execute= $ConnectingDB->query($sql);
	
		if ($Execute) {
			echo '<script>window.open("view_from_Database.php?id= Record Update Successfull","_self")</script>';
		}
